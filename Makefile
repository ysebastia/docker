...
molecule_os:
	...
	podman build --no-cache src/molecule-ubuntu --build-arg https_proxy=$(HTTP_PROXY) --build-arg http_proxy=$(HTTP_PROXY) --security-opt label=disable --build-arg BASE_OS=ubuntu --build-arg VERSION_OS=noble -t docker.io/ysebastia/molecule:noble
	podman build --no-cache src/molecule-ubuntu --build-arg https_proxy=$(HTTP_PROXY) --build-arg http_proxy=$(HTTP_PROXY) --security-opt label=disable --build-arg BASE_OS=ubuntu --build-arg VERSION_OS=jammy -t docker.io/ysebastia/molecule:jammy
