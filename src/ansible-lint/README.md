...
```bash
podman run --rm -t -v "${PWD}":/app:Z -w /app docker.io/ysebastia/ansible-lint:25.8.1 ansible-lint -p /app
```
...
```
  stage ('Ansible lint') {
    agent {
      docker {
        image 'ysebastia/ansible-lint:25.8.1'
      }
    }
    steps {
...
ansible-lint:
  image: docker.io/ysebastia/ansible-lint:25.8.1
  stage: test
  tags:
    - docker
