...
```bash
docker run --rm -t -v "${PWD}":/app -w /app docker.io/ysebastia/ansible-lint:24.10.0 ansible-lint -p /app
```
...
```
  stage ('Ansible lint') {
    agent {
      docker {
        image 'ysebastia/ansible-lint:24.10.0'
      }
    }
    steps {
...
ansible-lint:
  image: docker.io/ysebastia/ansible-lint:24.10.0
  stage: test
  tags:
    - docker
